...
/**
 * This class is the simulator for the problem. The simulator takes in an action
...
public class Simulator {
    ...
    private int steps;
    /**
     * Whether to print progress messages or not Feel free to change this if you
     * don't want messages printed
     **/
    ...
    public State step(Action a) throws IllegalArgumentException {
        ...
        if (!actionValidForLevel(a)) {
            throw new IllegalArgumentException("ActionType A" + a.getActionType().getActionNo()
                    + " is an invalid action for problem level " + ps.getLevel());
        }

        if (steps > ps.getMaxT()) {
            if (verbose) {
                System.out.println("Max time steps exceeded: " + steps + " > " + ps.getMaxT());
            }
            outputSteps(false);
            return null;
        }
        ...
        if (verbose) {
            System.out.println("Step " + steps + ": performing A" + a.getActionType().getActionNo());
        }
    ...
    /**
     * Convert the probability of slipping on a given terrain with 50% tire pressure
     * into a probability list, of move distance versus current terrain and tire
     * pressure.
     *
     * @param slipProb probability of slipping on current terrain and 50% tire
     *                 pressure
     ...
     * @param pD     P(D | E)
     * @param priorE P(E)
     * @return numerator of the P(E | A, B, C, D) formula (still need to divide by
     *         sum over E)
     */
    ...
    private State performA7(Action a) {
        ...
        return currentState.changeCarAndDriver(a.getCarType(), a.getDriverType());
    }
    ...
    private State performA8(Action a) {
        return currentState.changeTireFuelAndTirePressure(a.getTireModel(), a.getFuel(), a.getTirePressure());
    }
